...
# def get_curriculum_trajectory(episode: int, total_episodes: int) -> str:
#     """Returns the trajectory type based on the current training phase."""
#     # Phase 1: 0% to 25% of training
#     if episode < 0.25 * total_episodes:
#         return "square"
#     # Phase 2: 25% to 50% of training
#     elif episode < 0.50 * total_episodes:
#         return "triangular"
#     # Phase 3: 50% to 75% of training
#     elif episode < 0.75 * total_episodes:
#         return "square_wave"
#     # Phase 4: 75% to 100% of training (Generalization)
#     else:
#         return "all"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Train VTD3 in headless PyBullet")
    # ADDED "all" and "curriculum" to choices
    parser.add_argument("--trajectory", type=str, default="curriculum", 
                        choices=["triangular", "square", "sawtooth", "square_wave", "all", "curriculum"])
    parser.add_argument("--episodes", type=int, default=2000) 
    parser.add_argument("--max_episode_steps", type=int, default=1000)
    parser.add_argument("--video-interval", type=int, default=50) 
    parser.add_argument("--seed", type=int, default=42)
...
def get_curriculum_trajectory(episode: int, total_episodes: int) -> str:
    """Returns the trajectory type based on the current training phase."""
    # Phase 1: 0% to 25% of training (Easy straight lines and 90-deg corners)
    if episode < 0.25 * total_episodes:
        return "square"
    # Phase 2: 25% to 50% of training (Medium sharp angles)
    elif episode < 0.50 * total_episodes:
        return "triangular"
    # Phase 3: 50% to 75% of training (Hard continuous sharp turns)
    elif episode < 0.75 * total_episodes:
        return "square_wave"
    # Phase 4: 75% to 100% of training (Expert Generalization across all shapes)
    else:
        return "all"
...
def main() -> None:
    ...
    # Initialize environment
    # Pass the initial trajectory mode, though it may be overwritten dynamically in the loop
    env = DroneTrackingEnv(cfg=None, trajectory_mode=args.trajectory, GUI_mode=False)
    ...
    print(f"=== Starting Training on Device: {cfg.device} ===")
    print(f"=== Trajectory Mode: {args.trajectory.upper()} ===")
    
    # Start the Three-Stage Training Strategy 
    for ep in range(cfg.total_episodes): 
        
        # --- NEW: Trajectory Update Logic ---
        if args.trajectory == "curriculum":
            env.trajectory_mode = get_curriculum_trajectory(ep, cfg.total_episodes)
        else:
            env.trajectory_mode = args.trajectory
            
        # Reset environment
        state, info = env.reset(seed=cfg.seed + ep) #  
        
        # Extract the ACTUAL trajectory run this episode (critical if mode is "all")
        # Defaults to env.trajectory_mode if your env doesn't return it in info yet.
        current_traj = info.get("current_trajectory", env.trajectory_mode)
        # ------------------------------------
        ...
        while not (done or truncated):
            ...
            # ONLY pass 'done', never 'truncated', to the replay buffer to avoid Bellman zero-traps.
            replay.add(state, action, reward, next_state, float(done))
        ...
        if writer is not None:
            writer.add_scalar("train/reward_overall", ep_reward, ep)
            writer.add_scalar(f"train/reward_{current_traj}", ep_reward, ep) # Track specific shapes
            writer.add_scalar("train/error", avg_track_err, ep)
            if ep_actor_loss:
                writer.add_scalar("train/actor_loss", np.mean(ep_actor_loss), ep)
                writer.add_scalar("train/critic_loss", np.mean(ep_critic_loss), ep)
        ...
        if (ep + 1) % 25 == 0:
            agent.save(os.path.join(cfg.checkpoint_dir, f"ep_{ep + 1:04d}.pt"))
            print(f"Episode {ep + 1:04d}/{cfg.total_episodes} | Traj: {current_traj:11s} | Reward: {ep_reward:9.3f} | Error: {avg_track_err:7.4f}")
