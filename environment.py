...
class TargetTrajectory:
    ...
    def __init__(self, mode: str):
        self.mode = mode
        self.segment_duration = 6.5   # Time to travel between two waypoints
        self.pause_duration = 0.5      # Time target stays still at waypoints
    ...
    def sample(self, t: float) -> Tuple[np.ndarray, np.ndarray]: # returns (position, velocity)
        """
        ...
        cycle = self.duration() # Total time for one loop of the trajectory
        t_mod = t % cycle

        segment_full = self.segment_duration + self.pause_duration # Total time for one segment including pause
        
        
        seg_idx = min(int(np.floor(t_mod / segment_full)), self.n_segments - 1) # this tells us which segment we are in (0 to n_segments-1)

        t_local = t_mod - seg_idx * segment_full # Time in this segment (0 to segment_full)
        
        p0, p1 = self.points[seg_idx], self.points[seg_idx + 1] # Start and end points of the current segment
        

        # Handle target pausing at waypoints
        if t_local >= self.segment_duration: 
            return p1.copy(), np.zeros(3, dtype=np.float32) # Target is stationary at the waypoint,so return the position of the next waypoint and zero velocity

        # Solve for trapezoidal motion parameters
        # acceleration phase: 0 to ta, constant speed phase: ta to (ta+tc), deceleration phase: (ta+tc) to segment_duration
        T = self.segment_duration
        ta, tc, td = 0.25 * T, 0.5 * T, 0.25 * T  # Phase durations
        d = p1 - p0 # Vector from start to end point
        dist = np.linalg.norm(d)
        direction = d / (dist + 1e-8) # Unit vector in the direction of motion

        # Solve for vmax so the integral of velocity equals the distance
        vmax = dist / (0.5 * ta + tc + 0.5 * td + 1e-8) # Total distance = area under the velocity-time curve (trapezoidal profile)
        a = vmax / (ta + 1e-8) # Constant acceleration during the first phase (0 to ta) and deceleration during the last phase (ta+tc to segment_duration)
...
class DroneTrackingEnv(gym.Env):
    ...
    def __init__(self, cfg: Optional[EnvConfig] = None, trajectory_mode: str = "triangular"):
        ...
        self._client = p.connect(p.GUI) # Use p.GUI for visualization during development, but DIRECT is faster for training
        p.setAdditionalSearchPath(pybullet_data.getDataPath()) # To load plane.urdf and other assets
        p.setGravity(0.0, 0.0, -9.81) 


        # Load environment assets
        self.plane_id = p.loadURDF("plane.urdf", basePosition=[0, 0, 0], globalScaling=20) # Large flat ground
